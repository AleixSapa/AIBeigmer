...
def delete_model(model_id:str):
 global models_store
 before=len(models_store);models_store=[m for m in models_store if m['id']!=model_id]
 if len(models_store)==before: raise HTTPException(404,'Model no trobat')
 return {'ok':True}
...
async def run_execution(execution_id:str):
 ...
 q=next((q for q in questions_store if q['id']==x['question_id']),None);m=next((m for m in models_store if m['id']==x['model_id']),None)
 ...
 try:
  response,tokens=await generate(m,prompt);score,passed,total=evaluate(q,response)
