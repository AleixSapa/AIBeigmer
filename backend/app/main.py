...
app = FastAPI(title="AIBeigmer API", version="1.1.0", description="Real AI benchmarking platform")
...
async def discover(item: ProviderDiscoverIn):
    ...
    if item.provider == "Anthropic":
        headers = {"x-api-key": item.api_key, "anthropic-version": "2023-06-01"}
    if item.provider == "Google":
        headers, params = {}, {"key": item.api_key}
    try:
    ...
    for raw in data if isinstance(data, list) else []:
        ...
        if not mid:
            continue
        if catalog and not any(normal(mid) in normal(c) or normal(c) in normal(mid) for c in catalog if len(normal(c)) >= 4):
            continue
...
@app.post("/api/models")
async def legacy_discover(item: ProviderDiscoverIn):
    return await discover(item)
...
def delete_model(model_id: str):
    with engine.begin() as db:
        result = db.execute(text("DELETE FROM models WHERE id=:id"), {"id": model_id})
    if not result.rowcount:
        raise HTTPException(404, "Model no trobat")
    return {"ok": True}
...
async def generate(model, prompt):
    ...
    if not key:
        raise RuntimeError(f"Falta {PROVIDER_KEYS[provider]} al backend")
    async with httpx.AsyncClient(timeout=120) as client:
        if provider in {"OpenAI", "DeepSeek", "OpenRouter", "Groq"}:
            ...
            r = await client.post(urls[provider], headers={"Authorization": f"Bearer {key}"}, json={"model": model["model_id"], "messages": [{"role": "user", "content": prompt}], "temperature": 0})
            r.raise_for_status(); data = r.json()
            return data["choices"][0]["message"]["content"], data.get("usage", {}).get("total_tokens")
        if provider == "Anthropic":
            r = await client.post("https://api.anthropic.com/v1/messages", headers={"x-api-key": key, "anthropic-version": "2023-06-01"}, json={"model": model["model_id"], "max_tokens": 4096, "temperature": 0, "messages": [{"role": "user", "content": prompt}]})
            r.raise_for_status(); data = r.json()
            return "".join(x.get("text", "") for x in data.get("content", [])), data.get("usage", {}).get("input_tokens", 0) + data.get("usage", {}).get("output_tokens", 0)
        r = await client.post(f"https://generativelanguage.googleapis.com/v1beta/models/{model['model_id']}:generateContent?key={key}", json={"contents": [{"parts": [{"text": prompt}]}]})
        r.raise_for_status(); data = r.json()
        return data["candidates"][0]["content"]["parts"][0]["text"], data.get("usageMetadata", {}).get("totalTokenCount")
...
def evaluate(q, answer):
    if q["evaluation_type"] == "json_validation":
        try:
            json.loads(answer)
            return {"score": 100.0, "passed": 1, "total": 1, "details": {"valid_json": True}}
        except json.JSONDecodeError as exc:
            return {"score": 0.0, "passed": 0, "total": 1, "details": {"valid_json": False, "error": str(exc)}}
...
def persist_result(execution_id, evaluation):
    ...
    with engine.begin() as db:
        ...
        if evaluation["score"] is not None:
            db.execute(text("INSERT INTO scores(id,result_id,component,points,max_points,created_at) VALUES(:id,:rid,'evaluation',:points,100,:created)"), {"id": f"s-{execution_id}", "rid": result_id, "points": evaluation["score"], "created": now()})


@app.post("/api/executions", status_code=201)
def create_execution(item: ExecutionIn):
    if not one("SELECT id FROM questions WHERE id=:id AND active=TRUE", {"id": item.question_id}):
        raise HTTPException(404, "Pregunta no trobada")
    if not one("SELECT id FROM models WHERE id=:id AND active=TRUE", {"id": item.model_id}):
        raise HTTPException(404, "Model no trobat o inactiu")
    eid = f"e-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S%f')}"
    with engine.begin() as db:
        db.execute(text("INSERT INTO executions(id,question_id,model_id,status,created_at) VALUES(:id,:q,:m,'pending',:created)"), {"id": eid, "q": item.question_id, "m": item.model_id, "created": now()})
...
async def run_execution(execution_id: str):
    ...
    if not execution:
        raise HTTPException(404, "Execució no trobada")
    q = one("SELECT * FROM questions WHERE id=:id", {"id": execution["question_id"]})
    model = one("SELECT * FROM models WHERE id=:id", {"id": execution["model_id"]})
    start = perf_counter()
    with engine.begin() as db:
        db.execute(text("UPDATE executions SET status='running',error=NULL WHERE id=:id"), {"id": execution_id})
    try:
        answer, tokens = await generate(model, q["question"])
        evaluation = evaluate(q, answer)
        elapsed = round(perf_counter() - start, 3)
        with engine.begin() as db:
            db.execute(text("UPDATE executions SET status='completed',response=:response,time_seconds=:time,tokens=:tokens,finished_at=:finished WHERE id=:id"), {"id": execution_id, "response": answer, "time": elapsed, "tokens": tokens, "finished": now()})
        persist_result(execution_id, evaluation)
    except Exception as exc:
        elapsed = round(perf_counter() - start, 3)
        with engine.begin() as db:
            db.execute(text("UPDATE executions SET status='error',error=:error,time_seconds=:time,finished_at=:finished WHERE id=:id"), {"id": execution_id, "error": str(exc), "time": elapsed, "finished": now()})
...
def execution_list(question_id: str | None = None, model_id: str | None = None):
    return rows("SELECT e.*,r.score,r.tests_passed,r.tests_total,r.evaluation_type,r.evaluator_details FROM executions e LEFT JOIN results r ON r.execution_id=e.id WHERE (:q IS NULL OR e.question_id=:q) AND (:m IS NULL OR e.model_id=:m) ORDER BY e.created_at DESC", {"q": question_id, "m": model_id})


@app.get("/api/results")
def results():
    return rows("SELECT r.*,e.question_id,e.model_id,e.response,e.time_seconds,e.tokens,e.error,e.created_at AS execution_created_at FROM results r JOIN executions e ON e.id=r.execution_id ORDER BY r.created_at DESC")
...
def result(result_id: str):
    ...
    item["scores"] = rows("SELECT component,points,max_points FROM scores WHERE result_id=:id ORDER BY component", {"id": result_id})
    return item


@app.get("/api/ranking")
def ranking():
    return rows("SELECT m.id AS model_id,m.name,m.provider,AVG(r.score) FILTER (WHERE r.score IS NOT NULL) AS global_score,AVG(e.time_seconds) AS average_time,COUNT(r.id) AS evaluated_results FROM models m LEFT JOIN executions e ON e.model_id=m.id LEFT JOIN results r ON r.execution_id=e.id GROUP BY m.id,m.name,m.provider ORDER BY global_score DESC NULLS LAST")


@app.get("/api/stats")
def stats():
    return {"executions": one("SELECT COUNT(*) AS n FROM executions")["n"], "scored_executions": one("SELECT COUNT(*) AS n FROM results WHERE score IS NOT NULL")["n"], "average_score": one("SELECT AVG(score) AS value FROM results WHERE score IS NOT NULL")["value"]}
...
async def run_full_benchmark(item: BenchmarkRunIn):
    if not one("SELECT id FROM models WHERE id=:id AND active=TRUE", {"id": item.model_id}):
        raise HTTPException(404, "Model no trobat o inactiu")
    qs = rows("SELECT id FROM questions WHERE active=TRUE ORDER BY category,id")
    results = []
    for q in qs:
        execution = create_execution(ExecutionIn(question_id=q["id"], model_id=item.model_id))
        results.append(await run_execution(execution["id"]))
