...
app=FastAPI(title='AIBeigmer API',version='0.7.0',description='AI benchmarking platform')
...
# SQLite persistence. Models survive backend restarts and browser reloads.
with engine.begin() as db:
    db.execute(text('''CREATE TABLE IF NOT EXISTS models (
        id TEXT PRIMARY KEY, name TEXT NOT NULL, provider TEXT NOT NULL,
        model_id TEXT NOT NULL, description TEXT DEFAULT '', context INTEGER,
...
def load_models():
    with engine.connect() as db:
        rows=db.execute(text('SELECT id,name,provider,model_id,description,context,active FROM models ORDER BY rowid')).mappings().all()
    return [dict(r) for r in rows]
models_store=load_models()

def save_model(model):
    with engine.begin() as db:
        db.execute(text('''INSERT INTO models(id,name,provider,model_id,description,context,active,created_at)
            VALUES(:id,:name,:provider,:model_id,:description,:context,:active,:created_at)
            ON CONFLICT(provider,model_id) DO UPDATE SET name=excluded.name,description=excluded.description,context=excluded.context,active=excluded.active'''),model)
...
def models():
    global models_store
    models_store=load_models(); return models_store
...
def create_model(item:ModelIn):
 ...
 data=item.model_dump(exclude={'api_key'});model={'id':f"{item.provider.lower()}-{item.model_id}",**data,'created_at':datetime.now(timezone.utc).isoformat()}
 saved=save_model(model); models_store=load_models(); return saved
...
async def discover_freellmapi(item:FreeLLMDiscoverIn):
 ...
 for raw in data:
  ...
  if not mid: continue
  existing=next((m for m in load_models() if m['provider']=='FreeLLMAPI' and m['model_id']==mid),None)
  if existing:
   with engine.begin() as db: db.execute(text('UPDATE models SET active=1 WHERE id=:id'),{'id':existing['id']});continue
  model={'id':f'freellmapi-{mid}','name':raw.get('name') or mid,'provider':'FreeLLMAPI','model_id':mid,'description':raw.get('description') or 'Model gratuït detectat automàticament per FreeLLMAPI','context':raw.get('context_length') or raw.get('context_window') or raw.get('max_model_len'),'active':True,'created_at':datetime.now(timezone.utc).isoformat()}
  added.append(save_model(model))
 models_store=load_models(); return {'provider':'FreeLLMAPI','base_url':base,'total_detected':len(data),'added':len(added),'models':added}
...
def update_model(model_id:str,item:ModelIn):
 ...
 os.environ[PROVIDER_KEYS[item.provider]]=item.api_key
 data=item.model_dump(exclude={'api_key'});data['id']=model_id;data['created_at']=datetime.now(timezone.utc).isoformat();save_model(data);return next(m for m in load_models() if m['id']==model_id)
...
async def run_full_benchmark(item:BenchmarkRunIn):
 ...
 results=[]
 for q in qs:
  created=create_execution(ExecutionIn(question_id=q['id'],model_id=m['id']));result=await run_execution(created['id']);results.append(result)
...
@app.get('/api/stats')
def stats():
 done=[e for e in executions if e.get('score') is not None];ranked=ranking()
 return {'average_score':round(sum(e['score'] for e in done)/len(done),1) if done else None,'leader':ranked[0]['model'] if ranked else None,'executions':len(executions),'fastest_model':None}
